...
-- 1. commission_policies 현재 데이터 확인 (실행 후 결과 확인)
SELECT * FROM commission_policies;

-- 2. id=1 데이터가 없으면 기본 정책 insert
-- (이미 있으면 SKIP, upsert로 안전하게 처리)
INSERT INTO commission_policies (
    ...
    sales_dealer_master_direct,
    sales_leader_regular,
    sales_leader_master_direct,
    exec_leader_master_override,
    exec_leader_master_direct,
    is_percentage
) VALUES (
    1,
    1000000,  -- 장례수수료 기준
...
ON CONFLICT (id) DO UPDATE SET
    base_margin = EXCLUDED.base_margin,
...
-- 3. 트리거 함수 재작성 (COALESCE 방어코드 포함, 쿠폰 캐시백 수정)
...
-- 5. 확인 메시지
SELECT 'OK: 트리거 및 정책 설정 완료' AS result;
