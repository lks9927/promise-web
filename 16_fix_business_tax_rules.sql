-- 사용료 및 딜러 수수료 정산 시, 수신자(딜러/팀장)의 사업자 여부(has_business)를 확인하여 세금 처리 분기
...
DECLARE
    ...
    v_dealer_has_business BOOLEAN;
    v_team_leader_has_business BOOLEAN;
    v_master_dealer_has_business BOOLEAN;
...
BEGIN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'in_progress' THEN
        ...
        IF v_dealer_id IS NOT NULL THEN
            ...
            -- 딜러의 사업자 여부 확인
            SELECT COALESCE(has_business, false) INTO v_dealer_has_business
            FROM profiles WHERE id = v_dealer_id;
        END IF;

        -- 마스터 딜러의 사업자 여부 확인
        IF v_master_dealer_id IS NOT NULL THEN
            SELECT COALESCE(has_business, false) INTO v_master_dealer_has_business
            FROM profiles WHERE id = v_master_dealer_id;
        END IF;
        ...
        IF v_team_leader_id IS NOT NULL THEN
            ...
            -- 팀장의 사업자 여부 확인 (본사 사용료 입금 주체)
            SELECT COALESCE(has_business, false) INTO v_team_leader_has_business
            FROM profiles WHERE id = v_team_leader_id;
        END IF;

        -- A. 팀장 → 본사 사용료 (usage_fee는 부가세 포함가. 본사가 팀장에게서 받는 돈)
        -- 사용료는 본사의 매출이므로 팀장의 사업자 여부와 관계없이 무조건 부가세 10% 대상(본사 발행)
        ...
        -- B. 본사 → 딜러 수수료
        IF v_dealer_id IS NOT NULL THEN
            IF v_dealer_has_business THEN
                -- 딜러가 사업자인 경우: 수수료에 10% 부가세를 더해서 지급 (세금계산서 수취)
                v_tax_amt := ROUND(v_dealer_comm * 0.1);
                v_net_amt := v_dealer_comm + v_tax_amt;
                INSERT INTO settlements (case_id, recipient_id, amount, type, status, tax_type, base_amount, tax_amount, net_amount)
                VALUES (NEW.id, v_dealer_id, v_net_amt, 'dealer_commission', 'draft', 'vat_10', v_dealer_comm, v_tax_amt, v_net_amt);
            ELSE
                -- 딜러가 개인(프리랜서)인 경우: 수수료에서 3.3% 원천징수
            ...
            IF v_master_dealer_id IS NOT NULL AND v_master_dealer_id != v_dealer_id THEN
                IF v_master_dealer_has_business THEN
                    v_tax_amt := ROUND(v_dealer_override * 0.1);
                    v_net_amt := v_dealer_override + v_tax_amt;
                    INSERT INTO settlements (case_id, recipient_id, amount, type, status, tax_type, base_amount, tax_amount, net_amount)
                    VALUES (NEW.id, v_master_dealer_id, v_net_amt, 'dealer_override', 'draft', 'vat_10', v_dealer_override, v_tax_amt, v_net_amt);
                ELSE
...
SELECT 'OK: 딜러/팀장 사업자 유무에 따른 부가세/원천징수 분기 로직 적용 완료' AS result;
