-- ============================================================
-- 정산 프로세스 개선: 상담완료 시점 정산 레코드 생성
-- Supabase SQL Editor에서 실행하세요
-- ============================================================

-- 1. settlements 상태 제약조건 업데이트 (draft, awaiting_payment 추가)
...
ALTER TABLE settlements ADD CONSTRAINT settlements_status_check
    CHECK (status IN ('draft', 'pending', 'awaiting_payment', 'paid', 'cancelled'));

-- 2. 트리거 함수 재작성: in_progress 시점에 draft 정산 레코드 생성
...
BEGIN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'in_progress' THEN
        ...
        IF NEW.usage_fee > 0 AND v_team_leader_id IS NOT NULL THEN
            ...
            VALUES (NEW.id, v_team_leader_id, v_net_amt, 'usage_fee_remittance', 'draft', 'vat_10', NEW.usage_fee, v_tax_amt, v_net_amt);
        END IF;
...
-- 4. 확인
SELECT 'OK: 정산 프로세스 업데이트 완료 (in_progress → draft 생성)' AS result;
