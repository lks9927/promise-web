-- ============================================================
-- 12_fix_advanced_usage_fee.sql
-- 고급형 패키지의 입금액(usage_fee)이 88만 원으로 잘못 저장되는 문제 해결
-- 프론트엔드가 보낸 값을 무시하고 DB 정책 테이블(base_margin)을 강제 적용하도록 변경
-- ============================================================
...
BEGIN
    ...
    IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'in_progress' THEN
        ...
        -- 수수료 정책 로드 (base_margin 포함)
        SELECT
            COALESCE(base_margin, 880000),
        ...
        v_base_margin      := COALESCE(v_base_margin,      880000);
        ...
        -- 🚨 핵심 수정: 프론트엔드가 보낸 usage_fee를 무시하고 DB 정책 값(base_margin)으로 강제 덮어씀
        NEW.usage_fee := v_base_margin;
...
-- 기존 AFTER 트리거 삭제 후, NEW 값 변경이 가능한 BEFORE 트리거로 생성
DROP TRIGGER IF EXISTS trigger_calculate_settlements ON funeral_cases;
CREATE TRIGGER trigger_calculate_settlements
    BEFORE UPDATE ON funeral_cases
    FOR EACH ROW
    EXECUTE FUNCTION calculate_settlements();

-- 🚨 이미 잘못 들어간 과거 정산 데이터 및 장례건 사용료 일괄 수정 (고급형: 660,000, 무빈소: 440,000)
UPDATE funeral_cases SET usage_fee = 660000 WHERE package_name = '고급형' AND usage_fee != 660000;
UPDATE funeral_cases SET usage_fee = 440000 WHERE package_name = '무빈소' AND usage_fee != 440000;

-- settlements 내 입금 내역 수정
UPDATE settlements s
SET amount = c.usage_fee,
    net_amount = c.usage_fee,
    base_amount = ROUND(c.usage_fee / 1.1),
    tax_amount = c.usage_fee - ROUND(c.usage_fee / 1.1)
FROM funeral_cases c
WHERE s.case_id = c.id
  AND s.type = 'usage_fee_remittance'
  AND (c.package_name = '고급형' OR c.package_name = '무빈소');

SELECT 'OK: 입금액 로직 DB 강제 적용 및 기존 오류 데이터 복구 완료' AS result;
