...
export default function SendMessageModal({ isOpen, onClose, recipientId, recipientName, recipientRoleClass, currentUserId }) {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            // Supabase 에러 코드별 안내 메시지
            let errMsg = error.message || '알 수 없는 오류가 발생했습니다.';
            if (error.code === '23503') errMsg = '수신자 정보를 찾을 수 없습니다. (DB 설정 확인 필요)';
            else if (error.code === '42501') errMsg = '권한이 없습니다. (RLS 정책 확인 필요)';
            showToast('error', '발송 실패', errMsg);
        } finally {
            setIsSending(false);
        }
    };
