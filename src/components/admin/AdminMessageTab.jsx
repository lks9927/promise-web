...
import { sendWebPush } from '../../lib/pushNotification';

export default function AdminMessageTab({ partners, isReadonly }) {
    ...
    const handleSend = async (e) => {
        ...
        try {
            ...
            if (targetType === 'all') {
                ...
                targetUserIds = partners.filter(p => p.master_id === targetMasterId || p.user_id === targetMasterId).map(p => p.user_id);
            ...
            const notificationsToInsert = targetUserIds.map(userId => ({
                user_id: userId,
                sender_id: adminId, // Record Admin as sender
                type: 'info',
                title: messageTitle,
            ...
            // 📱 Web Push: 대기/백그라운드 상태 유저에게도 OS 알림 발송
            sendWebPush(targetUserIds, messageTitle, messageContent).catch(
                err => console.warn('[Push] 백그라운드 푸시 실패 (무시):', err)
            );
            ...
            <form onSubmit={handleSend} className="p-6 space-y-6">
                ...
                {isReadonly ? (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
                        <p className="text-sm font-bold text-amber-700">🔒 열람 전용 모드: 메시지 발송은 슬룰퍼 관리자만 가능합니다.</p>
                    </div>
                ) : (
                <button
                    type="submit"
